...
# hämtar information med opinionsundersökningar från API och sparar i tabell
df = pandas.read_csv(
    'https://raw.githubusercontent.com/hjnilsson/SwedishPolls/master/Data/Polls.csv')

wb = openpyxl.load_workbook('valsim.xlsx')  # öppnar excelfilen

# använder den aktiva sidan då excelfilen bara innehåller en sida
...
for i in range(0, len(partier_info)):
    # lägger till partiledare för varje parti
    partier_info[i]['Partiledare'] = partiledare[i]
    ...
    if vald_sida == "x":  # om användaren vill sluta programmmet rensas terminalen och whileloopen upphör
        ...
        for parti in partier_info:
            ...
            for parti_vidare in partier_invalda:
                # kollar ifall partiet matchar med det som gick vidare
                if parti['Partinamn'] == parti_vidare['Partinamn']:
                    vidare = True
        ...
        # gör kopia av listan för enbart denna sidan så att den ordinarie listan inte påverkas
        partier_invalda_maxröst = partier_invalda
        ...
        # de partiledare som tillhör det vinnande blocket skrivs ut i meddelandet
        ui.echo(glada_partiledare +
                " är glada över att vara en del av det största blocket.")

        ui.echo(partier_invalda_maxröst[0]['Partiledare'] +
                " är väldigt nöjd med det strålande valresultatet för " + partier_invalda_maxröst[0]['Partinamn'] + ".")
        ui.echo(partier_invalda_maxröst[-1]['Partiledare'] +
                " är missnöjd med bottenresultatet för " + partier_invalda_maxröst[-1]['Partinamn'] + ".")
        ui.echo(" H - Hem")
        ui.echo(" X - Avsluta")
        # tar input och ändrar vald sida
        ...
        # skriver ut första institutet i kolumnen Company, vilket är det senaste resultatet
        ui.echo("Institut: " + df['Company'][0])
        ...
        while klar == False:  # all kod som körs vid ny slumpnning. Även kod för de andra sidorna. Fördelen är att ingen kod behöver köras flera gånger i onödan.
            ...
            for parti in partier_invalda:
                # ger varje parti röster i procent som heltal
                procent_röster = int(
                    parti['Röster'] / antal_röster_invalda_partier * 100)
                parti['Procent_röster'] = procent_röster

                # räknar ut andelen resultat jämfört med det bästa möjliga resultatet
                parti['Andel_maxresultat'] = parti['Röster'] / \
                    parti['Maximum_röst']

                for block in Block_info:
                    if parti['Block'] == block['Block']:
                        # lägger till partiets storlek till rätt block
                        block['Storlek'] += parti['Procent_röster']
            ...
            # skapar del av meddelande med de partiledare som var nöjda med valresultatet
            for i in range(0, len(Block_info[0]['Partiledare'])):
                if i == len(Block_info[0]['Partiledare']) - 2:
                    # lägger till "och" efter den näst sista partiledaren
                    glada_partiledare += Block_info[0]['Partiledare'][i] + " och "
                elif i != len(Block_info[0]['Partiledare']) - 1:
                    # lägger till "," om det inte är sista eller näst sista partiledaren
                    glada_partiledare += Block_info[0]['Partiledare'][i] + ", "
                else:
                    # lägger inte till något efter sista partiledaren
                    glada_partiledare += Block_info[0]['Partiledare'][i]
        ...
        ui.echo(Block_info[0]['Block'] + " är det största blocket med " +
                str(Block_info[0]['Storlek']) + " procent av rösterna.")

        ui.echo("En majoritet " + Inriktning_info[0]['Inriktning'] + "orienterade blev invalda i riksdagen och fick totalt " + str(
            Inriktning_info[0]['Storlek']) + " av rösterna.")
