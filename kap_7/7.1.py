...
try:
    # kollar så att inputen är ett heltal
    valt_nummer = int(input("Ange en multiplikationstabell>>>"))
except ValueError:
...
if antal_gånger != 0:  # kör enbart while-loopen om ett korekkt tal skrvits in
    while antal_gånger <= 3:
        ...
        if antal_gånger == 3:
            # frågar efter 3 tal om användaren vill fortsätta
            fortsätta = input("Vill du fortsätta? (ja/nej)").lower()
            if fortsätta == "ja":
